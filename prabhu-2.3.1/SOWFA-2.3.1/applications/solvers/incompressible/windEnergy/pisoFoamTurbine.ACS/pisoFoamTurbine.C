...
Description
    Transient solver for incompressible flow with actuator sector  turbine model
...
#include "interpolateSplineXY.H" //---new line
...
int main(int argc, char *argv[])
{
    ...
    // Enter the time loop
    Info << "\nStarting time loop\n" << endl;
    Info << "this is the actuator sector" << endl;


    while (runTime.loop())   //controlled by controlDict
    {
        ...
        // PISO algorithm
        {
	     ...
	     cout<<" checking if the compiler works " << endl ; 

            UEqn.relax();
         //   Info<<"U in cell 164 ="<<U.internalFiled()[164]<<endl;

/*
            SolverPerformance test;
            test = UEqn.solve();
