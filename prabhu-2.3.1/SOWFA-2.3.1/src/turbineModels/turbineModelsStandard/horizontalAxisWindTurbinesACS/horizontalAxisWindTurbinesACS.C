...
{   
    ...
    forAll(turbineName,i)
    {
        ...
        numLinePoints.append(int(readScalar(turbineArrayProperties.subDict(turbineName[i]).lookup("numLinePoints"))));
        numLines.append(int(readScalar(turbineArrayProperties.subDict(turbineName[i]).lookup("numLines"))));  // number of lines in a sector
        cout<< "number of lines" <<  numLines[i] << "  "  << endl ;
        sectorAngle.append(scalar(readScalar(turbineArrayProperties.subDict(turbineName[i]).lookup("sectorAngle"))));
                           ...
                           if (k > 0)
                           {
                              ...
                              {  
                                 ...
                                 // Info
                                  ...
                                  sectorPoints[i][k][lineId][m] = rotatePoint(sectorPoints[i][k][lineId][m], rotorApex[i], uvShaft[i], (360.0/NumBl[j])*k*degRad); //replace it with elementAngle later 
