...
tmp<volSymmTensorField> GenEddyViscABL::devReff() const
{
    return -nuEff()*dev(twoSymm(fvc::grad(U())));
}


tmp<fvVectorMatrix> GenEddyViscABL::divDevReff(volVectorField& U) const
{
    return
    (
...
tmp<fvVectorMatrix> GenEddyViscABL::divDevRhoReff
(
